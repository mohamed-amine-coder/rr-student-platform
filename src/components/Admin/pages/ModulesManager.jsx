...
import React, { useState, useEffect } from 'react';
// تمت إضافة deleteDoc هنا
import { collection, getDocs, doc, setDoc, updateDoc, deleteDoc } from 'firebase/firestore';
import { db } from '../../../../firebase'; 
// تمت إضافة أيقونة Trash2
import { BookOpen, Plus, Edit2, X, Loader2, Trash2 } from 'lucide-react';
...
const ModulesManager = () => {
  ...
  // حالة جديدة لخطوة التأكيد قبل الحذف
  ...
  const handleEditClick = (mod) => {
    setIsEditMode(true);
    setShowDeleteConfirm(false); // إغلاق التأكيد إذا كان مفتوحاً
    setModuleId(mod.id);
    setModuleTitle(mod.title || '');
    setModuleDesc(mod.description || '');
    setModulePrice(mod.price || '99');
  ...
  const handleCancelEdit = () => {
    setIsEditMode(false);
    setShowDeleteConfirm(false); // إعادة ضبط حالة التأكيد
    setModuleId('');
    setModuleTitle('');
    setModuleDesc('');
    setModulePrice('99');
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      alert("🗑️ تم حذف الموديل بنجاح!");
      handleCancelEdit(); // تفريغ الفورم والخروج من وضع التعديل
      fetchModules(); // تحديث اللائحة
    } catch (error) {
      ...
      <div className="grid md:grid-cols-3 gap-8">
        ...
        <div className={`md:col-span-1 bg-white p-6 rounded-2xl shadow-sm border-2 transition-colors h-fit ${isEditMode ? 'border-yellow-400' : 'border-slate-200'}`}>
          <div className="flex items-center justify-between mb-6">
            ...
            {isEditMode && (
              <button type="button" onClick={handleCancelEdit} className="text-slate-400 hover:text-red-500 p-1">
                <X size={20} />
              </button>
            )}
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* قسم الحذف (يظهر فقط في وضع التعديل) */}
            {isEditMode && (
              <div className="pt-4 mt-4 border-t border-slate-100">
                {!showDeleteConfirm ? (
                  // الخطوة الأولى: زر الحذف المبدئي
                  <button
                    type="button" // ضروري لمنع إرسال الفورم
                    onClick={() => setShowDeleteConfirm(true)}
                    className="w-full py-3 rounded-xl font-bold flex items-center justify-center gap-2 text-red-500 bg-red-50 hover:bg-red-100 transition-colors"
                  >
                    <Trash2 size={20} /> حذف هذا الموديل
                  </button>
                ) : (
                  // الخطوة الثانية: أزرار التأكيد
                  <div className="bg-red-50 p-4 rounded-xl border border-red-200">
                    <p className="text-sm text-red-700 font-bold text-center mb-3">هل أنت متأكد من الحذف النهائي؟ لا يمكن التراجع.</p>
                    <div className="flex gap-2">
                      <button
                        type="button" // ضروري لمنع إرسال الفورم
                        onClick={handleDelete}
                        disabled={isSaving}
                        className="flex-1 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg font-bold flex justify-center items-center gap-2"
                      >
                        {isSaving ? <Loader2 className="animate-spin" size={16} /> : 'نعم، احذف'}
                      </button>
                      <button
                        type="button" // ضروري لمنع إرسال الفورم
                        onClick={() => setShowDeleteConfirm(false)}
                        disabled={isSaving}
                        className="flex-1 py-2 bg-white text-slate-700 border border-slate-300 hover:bg-slate-100 rounded-lg font-bold"
                      >
                        إلغاء
                      </button>
                    </div>
